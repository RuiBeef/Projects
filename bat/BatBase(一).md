...
第二，与rem不同的是`::`后的字符在执行时不会回显，无论是否`echo on`打开命令行，因为命令行解析器不认为他是一个有效的命令行。
另外, rem 可以用于  config.sys  文件中。
## 目录切换 cd
### 1、<font color=red>同一个分区</font>的切换
格式：CD [盘符][路径]
`cd..`：返回上一级目录
`cd\`：返回根目录
在DOS命令中`.`、`..`和`\`都是表示相对路径的，`.`是当前目录，`..`是上一层目录，而`\`则表示根目录。
